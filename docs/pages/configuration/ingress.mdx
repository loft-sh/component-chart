...
## `tlsIssuer`
The `tlsIssuer` option expects a string with the name of the issuer for cert-manager.
...
ingress:
  tls: true
  tlsIssuer: step-issuer
  tlsIssuerGroup: certmanager.step.sm
  tlsIssuerKind: StepIssuer
  rules:
  - host: my-static-host.tld
  - host: my-static-host2.tld
```
...
## `tlsIssuerGroup`
The `tlsIssuerGroup` option expects a string with the name of the issuer group for cert-manager.


## `tlsIssuerKind`
The `tlsIssuerKind` option expects a string with the name of the issuer kind for cert-manager.
